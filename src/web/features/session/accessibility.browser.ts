...
type FrameTree = Readonly<{
  frame: Readonly<{ url: string; id: string }>;
  childFrames?: ReadonlyArray<FrameTree>;
}>;
type AxNode = Readonly<{ ignored: boolean; name?: Readonly<{ value?: string }> }>;
type CdpClient = Readonly<{ send: (method: string, parameters?: object) => Promise<unknown> }>;

const findFrame = (tree: FrameTree): string | undefined =>
...
const accessibleTaskNames = async () => {
  ...
  const { nodes } = (await client.send("Accessibility.getFullAXTree", { frameId })) as {
