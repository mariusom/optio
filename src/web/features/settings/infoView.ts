...
const aboutContent = (h: HtmlBuilder<Message>) => [
  groupedList(
...
const promptContent = (copyStatus: PromptCopyStatus, h: HtmlBuilder<Message>) =>
  groupedList(
    {
    ...
    [
      controlRow(
        [
          h.p(
            [h.Class("whitespace-pre-wrap break-words text-sm leading-relaxed select-text")],
            [templatePrompt],
          ),
          button(
            {
          ...
          h.p(
            [h.Role("status"), h.AriaLive("polite"), h.Class("text-sm text-muted-foreground")],
            [
              copyStatus === "failed"
...
const webMcpInstructions = (h: HtmlBuilder<Message>) =>
  h.details(
    [],
    [
      h.summary(
...
export const infoPage = (
  ...
  h.div(
    ...
    [
      ...
      page(
        { className: "pt-6" },
        kind === "About" ? aboutContent(h) : [promptContent(copyStatus, h), webMcpInstructions(h)],
        h,
      ),
    ],
  );
