...
import { isBooleanTrue, isScalarAnswerValid, toggleCheckboxOption } from "../../fields";
import {
  canRecordTask,
  currentTask,
  isSectionDone,
      ...
      return h.label(
        ...
        [
          ...
          section.kind === "rating"
            ...
            : choiceBody(label, false, checkMark(isSelected, h), h),
        ],
      );
    }),
  );
        ...
        [
          ...(counter
            ? [
                button(
                  {
                    onClick: Message.AdjustedCounter({
                      taskFieldId: section.id,
                      delta: -1,
                    }),
                    isDisabled: !valid || section.value === "" || Number(section.value) <= 0,
                    variant: "outline",
                    className: "h-11 w-11 shrink-0",
                    attributes: [h.AriaLabel(`Decrease ${section.name}`)],
                  },
                  "−",
                  h,
                ),
                ...
                button(
                  {
                    ...
                    isDisabled: !valid || Number(section.value) >= Number.MAX_SAFE_INTEGER,
                    variant: "outline",
                    className: "h-11 w-11 shrink-0",
                    attributes: [h.AriaLabel(`Increase ${section.name}`)],
                  },
                  "+",
                  h,
                ),
