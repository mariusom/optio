...
export const DuplicateTemplate = Command.define('DuplicateTemplate', {
  ...
  execute: ({ id }) =>
    Effect.gen(function* () {
      ...
      if (source === undefined) return yield* effectFailure('Failed to duplicate template. Please try again.')
      ...
      const fields = fieldRowsToDefs(fieldRows).map(field => ({ ...field, id: crypto.randomUUID() }))
      const copyId = crypto.randomUUID()
      const copyName = nextDuplicateName(source.name, templates.map(t => t.name))
      store.commit(
    ...
    }).pipe(Effect.catch(error => Effect.succeed(Message.FailedTemplateOp({ error: String(error) })))),
})
        ...
        target.isDefault === 1 && remaining.length > 0
          ? [
              events.templateDefaultSet({
                id: remaining.reduce((a, b) => (a.createdAt.getTime() <= b.createdAt.getTime() ? a : b)).id,
              }),
            ]
          : []
