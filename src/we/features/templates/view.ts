...
const kebabMenu = (template: TemplateSummary, h: HtmlBuilder<Message>) =>
  h.div([h.Class('dropdown dropdown-end self-center pr-1')], [
    h.div(
      [h.Tabindex(0), h.Class('btn btn-ghost btn-sm btn-circle'), h.AriaLabel(`Actions for "${template.name}"`)],
    ...
    h.ul(
      ...
      [
        ...(template.isDefault
          ...
          : [
              h.li([], [
                h.button([h.OnClick(Message.ClickedSetDefaultTemplate({ id: template.id }))], ['Set as Default']),
              ]),
            ]),
        h.li([], [
          h.button([h.OnClick(Message.ClickedDuplicateTemplate({ id: template.id }))], ['Duplicate']),
        ]),
        h.li([], [
          h.button(
            [
...
const docIcon = (classes: string, h: HtmlBuilder<Message>) =>
  h.svg(
    ...
    [
      ...
      h.line([h.Attribute('x1', '9'), h.Attribute('y1', '13'), h.Attribute('x2', '15'), h.Attribute('y2', '13')], []),
      h.line([h.Attribute('x1', '9'), h.Attribute('y1', '17'), h.Attribute('x2', '13'), h.Attribute('y2', '17')], []),
    ],
  )
...
export const templatesPage = (
  ...
  const list =
    model.templates.length === 0
      ? []
      : [
          h.div([h.Class('mx-auto w-full max-w-3xl px-4 pt-3')], [
            h.div(
              [
                h.Class(
            ...
            h.p([h.Class('px-1 pt-2 text-[11px] text-base-content/40')], [
  ...
  const empty =
    model.templates.length === 0
      ? [
          h.div([h.Class('flex min-h-full flex-col')], [
            h.div(
              ...
              [
                ...
                h.p([h.Class('mt-1 text-xs leading-relaxed text-base-content/60')], [
  ...
  return h.div([h.Class('min-h-full pb-[calc(4rem+env(safe-area-inset-bottom))]')], [
    ...empty,
    ...list,
...
const templateRow = (template: TemplateSummary, h: HtmlBuilder<Message>) =>
  h.keyed('div')(template.id, [
    h.Class('flex items-stretch bg-base-100 transition-colors'),
  ], [
    h.button(
      [
        h.Class('flex min-w-0 grow cursor-pointer flex-col items-start gap-0.5 px-4 py-3.5 text-left select-none active:bg-base-200/70'),
        h.OnClick(Message.ClickedTemplateRow({ id: template.id })),
        h.AriaLabel(`Edit template ${template.name}`),
      ],
      [
        h.span([h.Class('flex w-full items-center gap-2')], [
          h.span([h.Class('truncate text-sm font-semibold')], [template.name]),
          ...(template.isDefault
            ? [h.span([h.Class('badge badge-sm shrink-0 border-none bg-accent/15 font-medium text-accent')], ['Default'])]
            : []),
        ]),
        h.span([h.Class('text-xs text-base-content/60')], [
          fieldSummaryLine(template.fieldCount, template.requiredCount),
        ]),
        h.span([h.Class('text-[11px] text-base-content/40')], [`Updated ${formatTimestamp(template.updatedAt)}`]),
      ],
    ),
...
const createModal = (newName: string, h: HtmlBuilder<Message>) =>
  h.div([h.Class('modal modal-open')], [
    h.div([h.Class('modal-box max-w-sm rounded-box border border-base-300 p-5')], [
      h.h3([h.Class('text-base font-bold')], ['New Template']),
      h.input([
        h.Class('input input-bordered mt-3 w-full rounded-field bg-base-100 text-base focus:outline-none'),
        h.Value(newName),
      ...
      h.div([h.Class('modal-action mt-4 gap-2')], [
        h.button([h.Class('btn btn-ghost rounded-field'), h.OnClick(Message.CanceledCreateTemplate())], ['Cancel']),
        h.button(
          [
...
const deleteModal = (pending: { readonly id: string; readonly name: string }, h: HtmlBuilder<Message>) =>
  h.div([h.Class('modal modal-open')], [
    h.div([h.Class('modal-box max-w-sm rounded-box border border-base-300 p-5')], [
      h.h3([h.Class('text-base font-bold')], ['Delete?']),
      h.p([h.Class('mt-1.5 text-xs leading-relaxed text-base-content/70')], [
        `Are you sure you want to delete "${pending.name}"? This action cannot be undone.`,
      ]),
      h.div([h.Class('modal-action mt-4 flex-col gap-2 sm:flex-row')], [
        h.button(
          [
        ...
        h.button(
          [h.Class('btn btn-ghost btn-block rounded-field text-xs sm:flex-1'), h.OnClick(Message.CanceledDeleteTemplate())],
          ['Cancel'],
        ),
      ]),
    ]),
